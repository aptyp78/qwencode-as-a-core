...
def extract_quotes(text, url):
    """Экстракция цитат с пост-проверкой cosine similarity.

    Этап 1: Извлечение через LLM с ролевыми ограничениями
    Этап 2: Пост-проверка: cosine similarity < 0.5 → отбросить (галлюцинация)
    """
